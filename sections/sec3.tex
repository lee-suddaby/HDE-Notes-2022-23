...
\begin{itemize}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		Consider the solution $\xt = c_1 e^{r_1t} \xib_1 + c_2 e^{r_2t} \xib_2$. \\
		We now observe how $x$ changes as $t \rightarrow \pm \infty$\\
		$x \rightarrow\infty$ as $t\rightarrow \pm \infty$ unless:\\
		$c_1 = 0$, in which case $x \rightarrow0$ as $t \rightarrow \infty$ \\
		$c_2 = 0$, in which case $x \rightarrow0$ as $t \rightarrow -\infty$ \\
	...
	\begin{enumerate}[label=(\roman*)]
		...
		The critical point in this case is a \vb{spiral sink} when $\lambda<0$ and \vb{spiral source} $r \rightarrow 0$ when $\lambda>0$ (see \Cref{fig:trajectory5}).
		
		\item $\lambda = 0$ \\
		With the equations in subcase (i) holding, we now have that $r'=0 \implies r$ is constant. So the critical point in this case is a \vb{centre}, as in \Cref{fig:trajectory6}.
	\end{enumerate}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\]
		The ratio $\frac{x}{y}$ is independent of $t$ but depends on the components of $\xib_1$ and $\xib_2$ and on $c_1, c_2$. The critical point in this case is a \vb{proper node} (see \Cref{fig:trajectory3}).
		
		\item One linearly independent eigenvector \\
		The solution is given by 
		\[
		\xt = c_1 e^{rt} \xib + c_2 e^{rt} (\xib t + \eta)
		\] The critical point in this case is an \vb{improper node} (see \Cref{fig:trajectory4}).
	\end{enumerate}
\end{itemize}
...
Furthermore, a critical point $\xta$ is \vb{asymptotically stable} if it is stable and $\exists \delta_0 >0$ such that $|\vbx(0) - \xta| < \delta_0$ implies $|\xt - \xta|<\varepsilon$ as $t \to \infty$. A graphical representation of stability and asymptotic stability is given in \Cref{fig:stability}.
